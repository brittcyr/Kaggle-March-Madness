...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        for a in range(len(inputs)):
            z += inputs[a].output() * weights[a].get_value()
    ...
    def compute_doutdx(self, elem):
        ...
        if self.has_weight(elem):
            for a in range(len(inputs)):
                if elem == weights[a]:
                    d = inputs[a].output()*o*(1-o)
                    return d

        z = 0

        for a in range(len(inputs)):
            if self.isa_descendant_weight_of(elem, weights[a]):
                z += o*(1.0-o)   *  weights[a].get_value()  *  inputs[a].dOutdX(elem)
        return z
...
class PerformanceElem(DifferentiableElement):
    ...
    def dOutdX(self, elem):
        ...
        """
        d = (self.my_desired_val - self.my_input.output()) * self.my_input.dOutdX(elem)
        return d
...
def make_neural_net_challenging():
    ...
    w_1A = Weight('w_1A', random.uniform(-1,1))
    w_2A = Weight('w_2A', random_weight())
    w_A  = Weight('w_A', random_weight())
    w_1B = Weight('w_1B', random_weight())
    w_B  = Weight('w_B', random_weight())
    w_1C = Weight('w_1C', random_weight())
    w_AC = Weight('w_AC', random_weight())
    w_BC = Weight('w_BC', random_weight())
    w_C  = Weight('w_C', random_weight())
    w_AD = Weight('w_AD', random_weight())
    w_BD = Weight('w_BD', random_weight())
    w_1D = Weight('w_1D', random_weight())
    w_D  = Weight('w_D', random_weight())
    w_2E = Weight('w_2E', random_weight())
    w_CE = Weight('w_CE', random_weight())
    w_DE = Weight('w_DE', random_weight())
    w_E  = Weight('w_E', random_weight())

    A = Neuron('A', [i1,i2,i0], [w_1A,w_2A,w_A])
    B = Neuron('B', [i1,i0], [w_1B,w_B])
    C = Neuron('C', [i1,A,B,i0], [w_1C,w_AC,w_BC,w_C])
    D = Neuron('D', [A,B,i1,i0], [w_AD,w_BD,w_1D,w_D])
    E = Neuron('D', [i2,C,D,i0], [w_2E,w_CE,w_DE,w_E])
...
def make_neural_net():
    ...
    w_1A = Weight('w_1A', random_weight())
    w_2A = Weight('w_2A', random_weight())
    w_3A = Weight('w_3A', random_weight())
    w_A  = Weight('w_A', random_weight())
    w_1B = Weight('w_1B', random_weight())
    w_2B = Weight('w_2B', random_weight())
    w_3B = Weight('w_3B', random_weight())
    w_B  = Weight('w_B', random_weight())
    w_1C = Weight('w_1C', random_weight())
    w_2C = Weight('w_2C', random_weight())
    w_3C = Weight('w_3C', random_weight())
    w_C  = Weight('w_C', random_weight())
    w_AD = Weight('w_AD', random_weight())
    w_BD = Weight('w_BD', random_weight())
    w_CD = Weight('w_CD', random_weight())
    w_D  = Weight('w_D', random_weight())

    A = Neuron('A', [i1,i2,i3,i0], [w_1A,w_2A,w_3A,w_A])
    B = Neuron('B', [i1,i2,i4,i0], [w_1B,w_2B,w_3B,w_B])
    C = Neuron('C', [i1,i2,i5,i0], [w_1C,w_2C,w_3C,w_C])
    D = Neuron('D', [A,B,C,i0], [w_AD,w_BD,w_CD,w_D])
...
def evaluate(network, data, verbose=False):
    """Test the neural net on some given data."""
    results = []
    for datum in data:
